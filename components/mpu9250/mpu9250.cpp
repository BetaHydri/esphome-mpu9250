...
namespace esphome
{
  namespace mpu9250
  {
    ...
    {
      calib_btn_ = b;
      calib_btn_->add_on_press_callback([this]()
                                        { start_calibration(); });
    }
    ...
    void MPU9250Component::update()
    {
      ...
      if (this->read_bytes(ACCEL_XOUT_H, b, 6))
      {
        ...
        if (ax_s_)
          ax_s_->publish_state(ax_ * 9.81f);
        if (ay_s_)
          ay_s_->publish_state(ay_ * 9.81f);
        if (az_s_)
          az_s_->publish_state(az_ * 9.81f);
      }
      ...
      if (this->read_bytes(GYRO_XOUT_H, b, 6))
      {
        ...
        if (gx_s_)
          gx_s_->publish_state(gx_);
        if (gy_s_)
          gy_s_->publish_state(gy_);
        if (gz_s_)
          gz_s_->publish_state(gz_);
      }
      ...
      if (bus_read_bytes(this->bus_, AK8963_ADDR, AK8963_ST1, &st, 1) && (st & 0x01))
      {
        ...
        if (mx_s_)
          mx_s_->publish_state(mx_);
        if (my_s_)
          my_s_->publish_state(my_);
        if (mz_s_)
          mz_s_->publish_state(mz_);
      }
